        ...
        <ul class="kt-menu__nav ">
            <li class="kt-menu__item " aria-haspopup="true">
                <a href="{{ route('index') }}" class="kt-menu__link ">
                    ...
                    <span class="kt-menu__link-text">控制台-首页</span>
                </a>
            </li>
            <li class="kt-menu__item " aria-haspopup="true">
                <a href="{{ route('menu.index') }}" class="kt-menu__link ">
                    <i class="kt-menu__link-icon flaticon-home"></i>
                    <span class="kt-menu__link-text">菜单管理</span>
                </a>
            </li>

            <li class="kt-menu__section ">
                <h4 class="kt-menu__section-text">后台管理</h4>
                <i class="kt-menu__section-icon flaticon-more-v2"></i>
            </li>
            @foreach($menuTree as $topNode)
            <li class="kt-menu__item  @if(count($topNode->nodes) > 0) kt-menu__item--submenu @endif" aria-haspopup="true" @if(count($topNode->nodes) > 0) data-ktmenu-submenu-toggle="hover" @endif>
                <a target="{{$topNode->target}}" href="{{$topNode->action}}"  class="kt-menu__link @if(count($topNode->nodes) > 0) kt-menu__toggle @endif" >
                    <i class="kt-menu__link-icon {{$topNode->icon}}"></i>
                    <span class="kt-menu__link-text">{{$topNode->name}}</span>
                    @if(count($topNode->nodes) > 0)
                ...
                <div class="kt-menu__submenu "><span class="kt-menu__arrow"></span>
                    <ul class="kt-menu__subnav">
                        ...
                        @foreach($topNode->nodes as $secondNode)
                        <li class="kt-menu__item  @if(count($secondNode->nodes) > 0) kt-menu__item--submenu @endif" aria-haspopup="true" @if(count($secondNode->nodes) > 0) data-ktmenu-submenu-toggle="hover" @endif>
                            <a target="{{$secondNode->target}}" href="{{$secondNode->action}}" class="kt-menu__link @if(count($secondNode->nodes) > 0) kt-menu__toggle @endif">
                                <i class="kt-menu__link-bullet kt-menu__link-bullet--line"><span></span></i>
                                <span class="kt-menu__link-text">{{$secondNode->name}}</span>
                                @if(count($secondNode->nodes) > 0)
                                <i class="kt-menu__ver-arrow la la-angle-right"></i>
                                @endif
                            </a>
                            @if(count($secondNode->nodes) > 0)
                            <div class="kt-menu__submenu "><span class="kt-menu__arrow"></span>
                                <ul class="kt-menu__subnav">
                                    @foreach($secondNode->nodes as $thirdNode)
                                    <li class="kt-menu__item  @if(count($thirdNode->nodes) > 0) kt-menu__item--submenu @endif " aria-haspopup="true" @if(count($thirdNode->nodes) > 0) data-ktmenu-submenu-toggle="hover" @endif>
                                        <a target="{{$thirdNode->target}}" href="{{$thirdNode->action}}" class="kt-menu__link @if(count($thirdNode->nodes) > 0) kt-menu__toggle @endif ">
                                            <i class="kt-menu__link-bullet kt-menu__link-bullet--dot"><span></span></i>
                                            <span class="kt-menu__link-text">{{$thirdNode->name}}</span>
                                            @if(count($thirdNode->nodes) > 0)
                                            <i class="kt-menu__ver-arrow la la-angle-right"></i>
                                            @endif
                                        </a>
                                        @if(count($thirdNode->nodes) > 0)
                                        <div class="kt-menu__submenu "><span class="kt-menu__arrow"></span>
                                            <ul class="kt-menu__subnav">
                                                @foreach($thirdNode->nodes as $fourthNode)
                                                    <li class="kt-menu__item " aria-haspopup="true">
                                                        <a target="{{$fourthNode->target}}" href="{{$fourthNode->action}}" class="kt-menu__link ">
                                                            <i class="kt-menu__link-bullet kt-menu__link-bullet--line"><span></span></i>
                                                            <span class="kt-menu__link-text">{{$fourthNode->name}}</span>
                                                        </a>
                                                    </li>
                                                @endforeach
                                            </ul>
